...
func init() {
	sentence := os.Getenv("WORDLE_SENTENCE")
	if sentence == "" {
		sentence = "APPLE BRAIN CLOUD" // default
	}
...
<body>
    ...
    <script>
        ...
        async function newGame() {
            const response = await fetch('/api/index?new-game=1', { method: 'POST' });
        ...
        async function makeGuess() {
            ...
            const response = await fetch('/api/index?guess=1', {
                method: 'POST',
	...
	tmpl = template.Must(template.New("home").Parse(tmplStr))
}
...
func Handler(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodGet {
		...
		tmpl.Execute(w, struct{ Words []string }{Words: game.Words})
		return
	}
	if r.Method == http.MethodPost {
		r.ParseForm()
		if r.URL.Query().Get("new-game") == "1" {
			game.GameOver = false
			game.Won = false
